...
while ret:
    ...
    for key, car in cars.items():
        ...
        patch = cv2.rectangle(patch, start_point, end_point,
                              color=(100, 45, 255), thickness=2)
        patch = cv2.putText(patch, f'carId:{key}', start_point,
                            fontFace=4, fontScale=0.6, color=(0, 0, 0), thickness=1)

    patch = cv2.putText(patch, f'Car passed:{ct.count}', (
        20, 20), fontFace=4, fontScale=0.6, color=(0, 0, 0), thickness=1)
    patch = cv2.rectangle(patch, (det_zone[0], det_zone[1]), (
        det_zone[2], det_zone[3]), color=(78, 252, 3), thickness=1)
